import { systems, articleSystemImpacts } from '@/db/schema'
import { z } from 'zod'
...
export const systemsRouter = router({
  ...
  list: orgProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (search) {
        conditions.push(
          sql`(${systems.name} ILIKE ${`%${search}%`} OR ${systems.description} ILIKE ${`%${search}%`})`
        )
      }
  ...
  getById: orgProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const regulationImpacts = new Map<string, {
  ...
  delete: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db
        ...
        .where(eq(articleSystemImpacts.systemId, input.id))
   ...
   */
  getArticlesForSystem: orgProcedure
